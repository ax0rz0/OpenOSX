function(install_symlink source_path target)
    ...
    # Expect source_path like: usr/include/foo -> relative to CMAKE_INSTALL_PREFIX
    get_filename_component(source_dir "${source_path}" DIRECTORY)
    get_filename_component(source_name "${source_path}" NAME)

    set(install_dir "\${CMAKE_INSTALL_PREFIX}/${source_dir}")

    if(SL_COMPONENT)
        install(CODE "
            file(MAKE_DIRECTORY \"\$ENV{DESTDIR}/${install_dir}\")
            execute_process(
                COMMAND \${CMAKE_COMMAND} -E create_symlink \"${target}\" \"${source_name}\"
                WORKING_DIRECTORY \"\$ENV{DESTDIR}/${install_dir}\"
            )
            message(STATUS \"Installed symlink: ${source_path} -> ${target}\")

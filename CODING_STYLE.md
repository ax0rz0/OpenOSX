...
* Contributions to OpenOSX that have dependencies not present in this repo are welcome, but are still subject to the above rules. The `USE_HOST_SDK` parameter can be used to enable use of the active Apple SDK if a file is not found in the local repo. Please use it in all targets unless otherwise specified.

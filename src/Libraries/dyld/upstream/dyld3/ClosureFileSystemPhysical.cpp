...
#include <sys/stat.h>
#include <mach/mach.h>
#if !TARGET_OS_SIMULATOR && !TARGET_OS_DRIVERKIT && __has_include(<sandbox.h>)
  #include <sandbox.h>
...
bool FileSystemPhysical::getRealPath(const char possiblePath[MAXPATHLEN], char realPath[MAXPATHLEN]) const {
    ...
    forEachPath(possiblePath, ^(const char* aPath, unsigned prefixLen, bool& stop) {
        ...
        if ( fd != -1 ) {
            ...
            /* Fucking christ... */
            std::error_code ec;
            std::filesystem::path tmp = aPath;
            std::filesystem::path real = std::filesystem::absolute(aPath, ec);
            strlcpy(tempPath, real.c_str(), MAXPATHLEN);
            success = ec.value() == 0;
#endif
            ::close(fd);
            if ( success ) {
        ...
        if ( fd == -1 ) {
            ...
            if ( ::fstat(fd, &statBuf) != 0 ) {
                ...
                if (std::filesystem::exists(aPath)) {
                    std::filesystem::path absolute = std::filesystem::absolute(aPath);
                    strlcpy(tempPath, absolute.c_str(), MAXPATHLEN);
                    const char* realPathWithin = &tempPath[prefixLen];
                    if ( strcmp(path, realPathWithin) == 0 ) {
                        if ( realerPath != path )
                            realerPath[0] = '\0';

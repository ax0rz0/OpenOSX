...
bool status_handle_readable(struct status_line *status) {
	...
	case PROTOCOL_TEXT:
		while (true) {
			if (read_bytes > 0 && status->buffer[read_bytes - 1] == '\n') {
				status->buffer[read_bytes - 1] = '\0';
			}
			// getline() below can move the buffer, so this has to be
			// refreshed each pass rather than captured once up front.
			...
			if (read_bytes < 0) {
				...
				// End of file returns -1 without touching errno, so
				// testing errno alone spins here forever once the status
				// command has exited.
				status_error(status, "[status command exited]");
				return true;
			}
		}
	case PROTOCOL_I3BAR:

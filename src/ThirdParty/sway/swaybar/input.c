...
void update_cursor(struct swaybar_seat *seat) {
	...
	// Load before destroying: cursor_image points into the theme, so tearing
	// the old one down first leaves it dangling on either of the failure
	// paths below, and the next pointer event reads freed memory.
	...
	struct wl_cursor *cursor = wl_cursor_theme_get_cursor(theme, "default");
	if (!cursor) {
		// libwayland-cursor's built-in fallback is only named left_ptr, so a
		// missing theme on disk lands here rather than at the load above.

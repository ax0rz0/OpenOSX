												...
												IONotifier * notifier)
{
    ...
    do {
        ...
        if (uuidProperty != NULL && fs->_uuidString != NULL) {
            // The partition scheme publishes the GPT UUID uppercase while the
            // boot-uuid from /chosen is lowercase; compare case-insensitively.
            const char * a = uuidProperty->getCStringNoCopy();
            const char * b = fs->_uuidString->getCStringNoCopy();
            if (a && b && strcasecmp(a, b) == 0) {
                VERBOSE_LOG("existing UUID property matched (%s ~= %s)\n", a, b);
                matched = true;
                break;
            }
        }

...
/*
 * Constants absent from this source snapshot but referenced by the driver.
 *   kATADataIsValidMask: ATA IDENTIFY words 83/84 validity bits (15:14);
 *     data is valid when (word & 0xC000) == 0x4000.
 *   kIOATASMARTUserClientAccessMask: option bit distinguishing a SMART user
 *     client open from a normal open.  With the SMART user client removed, no
 *     opener ever sets it, so handleOpen always passes through to super.
 */
#ifndef kATADataIsValidMask
#define kATADataIsValidMask			0xC000
#endif
#ifndef kIOATASMARTUserClientAccessMask
#define kIOATASMARTUserClientAccessMask		0x00010000
#endif
